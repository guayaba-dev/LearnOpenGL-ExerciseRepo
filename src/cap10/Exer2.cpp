...
glm::mat4 lookAt(const glm::vec3 &eye, const glm::vec3 &center,
  ...
  translate[3][0] = -eye.x; // Fourth column, first row
  translate[3][1] = -eye.y;
  translate[3][2] = -eye.z;
...
void panningCamera(glm::vec3 &eye, int radius, glm::mat4 &view) {
  ...
  view = our_glm::lookAt(eye, cameraFront, glm::vec3(0, 1, 0));
}
...
void movingCamera(glm::vec3 &eye, glm::mat4 &view) {
  ...
  view = our_glm::lookAt(eye, eye + glm::vec3(cameraFront), glm::vec3(0, 1, 0));
}
